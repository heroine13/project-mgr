...
### 21.1 2026-05-21 修改记录

#### 21.1.1 项目详情页添加甘特图标签页

**修改文件**: `frontend/src/views/ProjectDetailView.vue`

**变更内容**:
- 使用 `el-tabs` 组件重构主内容区
- 新增“甘特图”标签页，集成 `GanttChart` 组件
- 新增“描述”标签页
- 添加甘特图数据加载逻辑
- 添加相关CSS样式

#### 21.1.2 项目内任务列表添加删除功能

**修改文件**: `frontend/src/views/ProjectDetailView.vue`

**变更内容**:
- 在任务列表的操作栏添加删除按钮
- 使用 `el-popconfirm` 实现二次确认
- 添加 `deleteTask` 方法，调用后端删除API
- 添加 `viewTask`、`createTask`、`editTask` 方法（之前缺失）

#### 21.1.3 创建任务页面支持从项目跳转

**修改文件**: `frontend/src/views/CreateTaskView.vue`

**变更内容**:
- 支持通过路由参数 `?projectId=xxx` 自动设置项目ID
- 修改 import 添加 `useRoute`
- 初始化表单时从路由参数读取 `project_id`
